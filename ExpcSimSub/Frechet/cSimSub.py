...
def trace_back(DP_list):
    ...
    while i > 0 and j > 0:
        ...
        if DP_idx == 0:
            ...
            three_way = np.argmin([prev_DP[i, j-1], prev_DP[i-1, j-1], cur_DP[i-1, j]])
            if three_way == 2:
                i = i - 1
            elif three_way == 1:
                j = j - 1
    ...
    if min_len == 1:
        ...
        for i in range(1, n):
            for j in range(1, m):
                ...
                DP[i, j] = max(min(DP[i-1, j], DP[i, j-1], DP[i-1, j-1]), p_dist)
    else:
        ...
        for i in range(min_len-1, m):
            ...
            DP[0, i] = max(prev_DP[0, i-1], p_dist)
        # Dynamic programming
        for i in range(1, n):
            for j in range(min_len-1, m):
                p_dist = np.linalg.norm(traj_query[i] - traj_data[j])
                DP[i, j] = max(min(DP[i-1, j], prev_DP[i-1, j-1], prev_DP[i, j-1]), p_dist)
...
if __name__ == "__main__":
    ...
    # save_path = './results/porto_frechet_cSimSub.pkl'
    ...
    results_record = {'Time': [], 'C_length': [], 'subsim': [], 'subtraj': []}
    ...
    for c_length in c_lengths:
        ...
        results_record['subtraj'].append(subtrajs)
